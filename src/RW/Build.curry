...
-- | Returns true if the given type declaration is polymorphic
--   (contains at least one type variable)
isPolymorphic :: CTypeDecl -> Bool
isPolymorphic (CType _ _ tvs _ _) = not $ null tvs

-- | Returns true if the given type declaration is monomorphic
...
--   Example:
--
--       unwrapApply (CTApply (CTApply (Cons "map") (Var "x")) (Var "y"))
--        = [Cons "map", Var "x", Var "y"]
...
-- | The type variable used as the placeholder for
--   instance declarations of the `ReadWrite` class.
...
--       data T t1 ... tn = ...
--
--   , this function can be used to derive all constraints
--   necessary to generate the instance header
--
...
returnTypeExpr True  te format
  = format ~> (CTApply (CTApply (CTCons ("Prelude", "(,)")) te) format)
returnTypeExpr False _  format
  = format ~> CTVar (0, "a")
...
showTypeExpr (CFuncType a b)    = "(" ++ showTypeExpr a
                                      ++ " -> "
                                      ++ showTypeExpr b ++ ")"
...
--   For `n` passed patterns, the resulting pattern is
--
--       p1:p2:...:pn
--
...
undefinedConstructorRule cons = case cons of
    (CCons name _ tes)
      -> CRule [CPComb name (map (\i -> CPVar (i, varName i)) (fromIndex0 tes))]
               (CSimpleRhs (CSymbol ("Huh?", "undefined")) [])
    _
      -> CRule []
               (CSimpleRhs (CSymbol ("Huh?", "fail")) [])
...
flatProgToAbstract (FCT.Prog name is ts _ _)
  = ACT.CurryProg name is Nothing
                  [] []
                  (filter (not . isPrefixOf "_Dict#" . typeDeclToName) (map flatTypeDeclToAbstract ts))
                  [] []
 where
  flatTypeDeclToAbstract t = case t of
    (FCT.Type qn vis tvar cds)
      -> ACT.CType qn (flatVisiblityToAbstract vis)
                      (map flatTypeVarToAbstract tvar)
                      (map flatConstrDeclToAbstract cds)
                      []
    (FCT.TypeNew qn vis tvar c)
      -> ACT.CType qn (flatVisiblityToAbstract vis)
                      (map flatTypeVarToAbstract tvar)
                      [flatNewConsToAbstractCons c] []
  ...
  flatConstrDeclToAbstract (FCT.Cons qn _ vis tes)
    = ACT.CCons qn (flatVisiblityToAbstract vis)
                   (map flatTypeExprToAbstract tes)

  flatNewConsToAbstractCons (FCT.NewCons qn vis te)
    = ACT.CCons qn (flatVisiblityToAbstract vis)
                   [flatTypeExprToAbstract te]

  flatTypeExprToAbstract fte = case fte of
    FCT.TVar index     -> ACT.CTVar (index, varName index)
    FCT.FuncType t1 t2 -> ACT.CFuncType (flatTypeExprToAbstract t1)
                                        (flatTypeExprToAbstract t2)
...
-- | If just a value is given, the predicate is applied to the value.
